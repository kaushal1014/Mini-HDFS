...
# --- 2. UPDATE send_chunk_to_datanode (for replication) ---
def send_chunk_to_datanode(datanode_host, datanode_port, datanode_id, chunk_id, chunk_data):
    ...
    try:
        with socket.socket(socket.AF_INET, socket.SOCK_STREAM) as s:
            ...
            # --- CALCULATE CHECKSUM FOR REPLICATION ---
            ...
            header = {
                ...
                "chunk_id": chunk_id,
                "size": len(chunk_data),
                "checksum": chunk_checksum # <-- Add checksum
            }
    ...
    try:
        ...
        if command == 'STORE_CHUNK':
            ...
            with open(chunk_path, 'wb') as f:
                f.write(buffer)
                m.update(buffer) # Add buffer to hash
                ...
                while bytes_received < chunk_size:
                    ...
                    if not data:
                        break
                    f.write(data)
                    m.update(data) # Add new data to hash
            ...
            if bytes_received != chunk_size:
                ...
                os.remove(chunk_path) # Delete corrupt file
